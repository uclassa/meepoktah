...
export default function Exco({ excoData }: { excoData: ExcoMemberData[] }) {
    return (
        <div
            id="team"
            className="flex flex-col items-center justify-center max-md:-mt-32"
        >
            ...
            {excoData?.length > 0 && (
                <div className="grid max-w-full justify-items-center gap-4 py-2.5 md:grid-cols-2 xl:grid-cols-3">
                    {excoData.map((data) => (
                        <ExcoMember {...data} key={data.id} />
                    ))}
                </div>
            )}
        </div>
    ...
    return (
        <div className="flex w-[320px] flex-col items-center overflow-hidden rounded-2xl bg-darkblue text-offwhite">
            <div className="top-0 grid h-[320px] grid-cols-1 grid-rows-1 items-center justify-items-center overflow-hidden">
                ...
                {data.alt_photo && (
                    <img
                        className="col-[1] row-[1] min-h-1/1 object-cover opacity-0 transition duration-300 ease-linear hover:opacity-100"
                        src={data.alt_photo}
                        alt={data.alt}
                    />
                )}
